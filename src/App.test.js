...
var mockServicesObject;
var spyOnGetStudents;
var spyOnGetCoursesForStudent;
var spyOnGetCourse;
...
jest.mock("./services/student.service", () => {
  return jest.fn().mockImplementation(() => {
    ...
    spyOnGetStudents = jest.spyOn(mockServicesObject, "getStudents");
    spyOnGetCoursesForStudent = jest.spyOn(
      mockServicesObject,
      "getCoursesForStudent"
    );
    spyOnGetCourse = jest.spyOn(mockServicesObject, "getCourse");
...
describe("Overall application tests", () => {
  beforeEach(() => {
    ...
    mockStudentCourses = server.getRandomStudentCourses(
      mockStudents,
      mockCourses
    );
  ...
  it("Should display the complete list of students with full name in the dropdown", () => {
    ...
    studentsOptions.forEach((option, index) => {
      expect(option.innerHTML.includes(mockStudents[index].firstName)).toEqual(
        true
      );
      expect(option.innerHTML.includes(mockStudents[index].lastName)).toEqual(
        true
      );
    });
  });
...
describe("Selecting a student tests", () => {
  ...
  it("should display the selected student's name, student number, email, and registration date", (done) => {
    ...
    const optionToSelect = Array.from(options).find((option) => {
      return (
        option.innerHTML.includes(mockStudentToSelect.firstName) &&
    ...
    setTimeout(() => {
      try {
        const studentInfoElements = Array.from(
          document.body.querySelectorAll(".student-info")
        );
        expect(studentInfoElements.length).toEqual(1);
        expect(
          studentInfoElements[0].innerHTML.includes(
            mockStudentToSelect.firstName
          )
        ).toEqual(true);
        expect(
          studentInfoElements[0].innerHTML.includes(
            mockStudentToSelect.lastName
          )
        ).toEqual(true);
        expect(
          studentInfoElements[0].innerHTML.includes(
            mockStudentToSelect.studentNumber
          )
        ).toEqual(true);
        expect(
          studentInfoElements[0].innerHTML.includes(mockStudentToSelect.email)
        ).toEqual(true);
        expect(
          studentInfoElements[0].innerHTML.includes(
            mockStudentToSelect.registrationDate.toLocaleDateString()
          )
        ).toEqual(true);

        notSelectedStudents.forEach((student) => {
          expect(
            !studentInfoElements[0].innerHTML.includes(student.firstName)
          ).toEqual(true);
          expect(
            !studentInfoElements[0].innerHTML.includes(student.lastName)
          ).toEqual(true);
          expect(
            !studentInfoElements[0].innerHTML.includes(student.studentNumber)
          ).toEqual(true);
          expect(
            !studentInfoElements[0].innerHTML.includes(student.email)
          ).toEqual(true);
          expect(
            !studentInfoElements[0].innerHTML.includes(
  ...
  it("should display the selected student's courses with the name, instructor, and credits - exact format", (done) => {
    ...
    setTimeout(() => {
      try {
        const coursesInfoElements = Array.from(
          document.body.querySelectorAll(".course-info")
        );
        ...
        coursesInfoElements.forEach((courseInfoElement) => {
          ...
          expect(
            courseInfoElement.innerHTML.includes(matchCourse.name)
          ).toEqual(true);
          expect(
            courseInfoElement.innerHTML.includes(matchCourse.instructor)
          ).toEqual(true);
          expect(
            courseInfoElement.innerHTML.includes(matchCourse.credits.toString())
          ).toEqual(true);
          ...
          expect(
            courseInfoElement.innerHTML
              .trim()
              .includes(expectedCourseText.trim())
          ).toEqual(true);

          studentCoursesNotTaken.forEach((course) => {
            expect(!courseInfoElement.innerHTML.includes(course.name)).toEqual(
              true
            );
            expect(
              !courseInfoElement.innerHTML.includes(course.instructor)
            ).toEqual(true);
          });
  ...
  it(`should display the selected student's courses in alphabetical order by course name`, (done) => {
    const courseIdsForSelectedStudent =
      mockStudentCourses[mockStudentToSelect.email];
    const courseNamesInAlphabeticalOrder = mockCourses
...
describe("No selection tests", () => {
  ...
  it("should not display student info when no student is selected", () => {
    ...
    expect(
      !studentInfoElements ||
        !studentInfoElements[0] ||
        !studentInfoElements[0].innerHTML.trim()
    ).toEqual(true);
  });
